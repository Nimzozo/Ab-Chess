...
// TODO :
// import / export variations / comments
...
window.AbChess = window.AbChess || function (containerId, abConfig) {
    ...
    function Chessgame() {
        ...
        the_game.playNthMove = function (moveIndex, move, promotion) {

            // Play a move on the nth position and store the resulting FEN
            // in the game if it's legal. Return the resulting FEN.
            // Do nothing if the move exists. Create a variation else.
            ...
            if (typeof moveIndex !== 'number' || moveIndex < 0 ||
                moveIndex > the_game.fenStrings.length - 1) {
                throw new Error(error.invalid_parameter);
            }
            if (!regex_move.test(move)) {
                throw new SyntaxError(error.invalid_parameter);
            }
            nthPosition = the_game.getNthPosition(moveIndex);
            if (nthPosition.checkMoveLegality(move)) {
                promotion = promotion || '';
                nextPosition = nthPosition.getNewPosition(move, promotion);
                if (nextPosition.isInCheck(nextPosition.activeColor)) {
                    stringToAdd = (nextPosition.hasLegalMoves())
                        ? '+'
                        : '#';
                }
                pgnMove = nthPosition.getPGNMove(move, promotion, false, stringToAdd);
                if (moveIndex < the_game.fenStrings.length - 1) {
                    nthMove = the_game.moves[moveIndex];
                    if (nthMove === move) { // TODO check promotion difference
                        return;
                    }
                    the_game.addVariation(moveIndex, move, pgnMove, nextPosition.fenString);
                } else {
                    the_game.fenStrings.push(nextPosition.fenString);
                    the_game.moves.push(move);
                    the_game.pgnMoves.push(pgnMove);
        ...
        the_game.checkVariationExistence = function (id) {
        ...
        the_game.getVariation = function (id) {

            // Return the desired variation.

            var key = 0;
            if (the_game.variations.some(function (variation, index) {
                if (variation.id === id) {
                    key = index;
                    return true;
                }
                ...
                return the_game.variations[key];
            }
            throw new Error(error.invalid_parameter);
        };

        the_game.checkMoveExistence = function (
            id,
            moveIndex,
            move) {

            // Return true if the move already exists in the game
            // (or in a variation).
            ...
            if (moveIndex < variation.moveIndex ||
                moveIndex > variation.moveIndex + variation.moves.length) {
                throw new Error(error.invalid_parameter);
            }
        ...
        the_game.setPGN = function (pgn) {
            ...
            while (regex_variation.test(pgn)) {
                importedVariations = regex_variation.exec(pgn);
            ...
            the_game.pgnMoves.forEach(function (move) {
                ...
                nextPosition = lastPosition.getNewPosition(simpleMove, promotion);
    ...
    function Variation(id, moveIndex, firstFEN) {

        // Variation constructor.
        // A variation is an object with properties :
        // - the id : its place in the variations tree.
        // - the move index where it begins.
        // - an array of moves.
        ...
        the_variation.addMove = function (move) {
            ...
            var pgnMove = '';
            var position = {};
            fen = the_variation.fenStrings[the_variation.fenStrings.length - 1];
            position = new Position(fen);
            // pgnMove = position.getPGNMove();
            nextPosition = position.getNewPosition(move);
            the_variation.length += 1;
            the_variation.moves.push(move);
            the_variation.fenStrings.push(nextPosition.fenString);
        };
